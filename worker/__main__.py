...
Runs on a fixed cycle:
  ...
  2. Coherent Engine: consume queued jobs
...
def run_job_consumer(status: WorkerStatus, max_jobs: int = 5, dry_run: bool = False) -> dict:
    """Consume queued coherent_engine jobs (if available)."""
    result = {"processed": 0, "error": ""}
    try:
        # Only run if coherent_engine is available in the full workspace
        ...
        from coherent_engine.pipeline.job_runner import run_job
        ...
        pass  # coherent_engine not available in OSS-only mode
    except Exception as e:
        result["error"] = f"{type(e).__name__}: {str(e)[:200]}"
